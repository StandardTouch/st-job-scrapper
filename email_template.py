...
This is an automated report generated by the scraping system.
        ...
        # Validate SMTP host (common mistake: smtp.google.com should be smtp.gmail.com)
        if 'google.com' in smtp_host.lower() and 'gmail' not in smtp_host.lower():
            logger.warning(f"SMTP host '{smtp_host}' might be incorrect. For Gmail, use 'smtp.gmail.com'")
            print(f"Warning: SMTP host '{smtp_host}' might be incorrect. For Gmail, use 'smtp.gmail.com'", flush=True)
            # Auto-correct for common mistake
            if smtp_host.lower() == 'smtp.google.com':
                logger.info("Auto-correcting SMTP host to smtp.gmail.com")
                print("Auto-correcting SMTP host to smtp.gmail.com", flush=True)
                smtp_host = 'smtp.gmail.com'
        
        # Test DNS resolution first with timeout
        logger.info(f"Resolving SMTP host {smtp_host}...")
        print(f"Resolving SMTP host {smtp_host}...", flush=True)
        try:
            socket.setdefaulttimeout(10)  # 10 second timeout for DNS
            socket.gethostbyname(smtp_host)
            logger.info("DNS resolution successful")
            print("DNS resolution successful", flush=True)
        except socket.gaierror as e:
            error_msg = f"Failed to resolve SMTP host '{smtp_host}': {str(e)}. Check your SMTP_HOST setting."
            logger.error(error_msg)
            print(error_msg, flush=True)
            raise Exception(error_msg)
        except socket.timeout:
            error_msg = f"DNS resolution timed out for '{smtp_host}'. Check your network connection."
            logger.error(error_msg)
            print(error_msg, flush=True)
            raise Exception(error_msg)
        
        # Set socket timeout for SMTP connection
        socket.setdefaulttimeout(15)  # 15 second timeout for SMTP operations
        
        # Send email via SMTP server with timeout
        logger.info(f"Connecting to SMTP server {smtp_host}:{smtp_port}...")
        print(f"Connecting to SMTP server {smtp_host}:{smtp_port}...", flush=True)
        
        try:
            server = smtplib.SMTP(smtp_host, smtp_port, timeout=15)  # 15 second timeout
            ...
            print(f"Sending email to {', '.join(recipient_emails)}...", flush=True)
            server.send_message(msg, to_addrs=recipient_emails)
            
            logger.info(f"Email report sent successfully to {', '.join(recipient_emails)}")
            print(f"Email report sent successfully to {', '.join(recipient_emails)}", flush=True)
            server.quit()
            return True
        except socket.timeout:
            error_msg = f"SMTP connection timed out after 15 seconds. Check your SMTP settings and network connection."
            logger.error(error_msg)
            print(error_msg, flush=True)
            raise Exception(error_msg)
        except TimeoutError:
            error_msg = f"SMTP operation timed out. Check your SMTP settings and network connection."
            logger.error(error_msg)
            print(error_msg, flush=True)
            raise Exception(error_msg)
        except socket.gaierror as e:
            error_msg = f"SMTP host resolution failed: {str(e)}. Check SMTP_HOST in your .env file."
            logger.error(error_msg)
            print(error_msg, flush=True)
            raise Exception(error_msg)
